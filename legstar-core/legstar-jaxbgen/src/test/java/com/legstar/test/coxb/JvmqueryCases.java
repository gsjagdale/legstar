...
public final class JvmqueryCases extends TestCase {
    ...
    /**
     * Check that data object contains the expected values. This assumes the
     * Locale is France
     * 
     * @param jvmQueryReply the java object to check
     */
    public static void checkJavaObjectReplyFrance(
            final JVMQueryReply jvmQueryReply) {
        ...
        assertTrue(System.getenv("JAVA_HOME").startsWith(
                jvmQueryReply.getEnvVarValues().get(0)));
        assertTrue(System.getenv("LEGSTAR_HOME").startsWith(
                jvmQueryReply.getEnvVarValues().get(1)));
    }
    ...
    public static void checkHostBytesHexReplyFrance(final String hostBytesHex) {
        ...
        assertEquals(JVMQUERYREPLY_HOST_BYTES.substring(0, 8),
                hostBytesHex.substring(0, 8));
        assertEquals(JVMQUERYREPLY_HOST_BYTES.substring(8, 40),
                hostBytesHex.substring(8, 40));
        assertEquals(JVMQUERYREPLY_HOST_BYTES.substring(40, 72),
                hostBytesHex.substring(40, 72));
